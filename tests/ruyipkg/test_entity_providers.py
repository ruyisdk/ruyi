...
def test_entity_store_with_custom_provider(ruyi_file: RuyiFileFixtureFactory) -> None:
    ...
    with ruyi_file.path("ruyipkg_suites", "entities_v0_smoke") as entities_path:
        ...
        fs_provider = FSEntityProvider(entities_path)
        mock_provider = MockEntityProvider()
        store = EntityStore(fs_provider, mock_provider)
