...
* `vendor` 提供了包的提供者相关信息。其中：
    ...
    - `data` 是可选的、供各厂商（vendor）存放各自私有元数据的命名空间。它是一个映射，其
      ...
      干扰，各自解读自己名下的内容。除下述保留的厂商标识符外，其余厂商标识符名下的内容仅
      作为无类型的键值对透传保存，不做额外的模式约束。
        - `ruyisdk`：保留给 RuyiSDK 官方使用的强类型元数据块。目前定义了以下字段：
            - `certified`：可选的布尔值。为 `true` 时，表示该包已通过 RuyiSDK 官方的人工
              测试，被标记为「RuyiSDK Certified」。其他情形等效于未认证。相关认证的溯源信
              息（谁在何时、针对哪个版本进行了测试等）由软件源的 Git 历史记录承载，故此处
              不再重复记录。
