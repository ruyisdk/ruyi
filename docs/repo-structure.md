...
与之类似，在某软件包目录 `packages/<category>/<name>/` 内部，任何以 `_`（下划线）
开头的条目（无论文件还是子目录）均为保留的辅助内容，不会被解释为版本定义。例如
`_abi/`、`_files/`、`_notes.toml` 等。Ruyi 运行时已经忽略这类条目，`ruyi admin check`
也接受它们的存在。
...
* `vendor` 提供了包的提供者相关信息。其中：
    ...
    - `data` 是可选的、供各厂商（vendor）存放各自私有元数据的命名空间。它是一个映射，其
      key 为厂商标识符（vendor ID），value 为该厂商自定义的元数据块。不同厂商的数据互不
      干扰，各自解读自己名下的内容。每个元数据块中各字段的值均须为字符串或布尔值，其他类型
      将被拒绝。除下述保留的厂商标识符外，其余厂商标识符名下的内容仅作为上述受限的键值对透
      传保存，不做额外的模式约束。
        - `ruyisdk`：保留给 RuyiSDK 官方使用的强类型元数据块。目前定义了以下字段：
            - `certified`：可选的布尔值。为布尔值 `true` 时，表示该包已通过 RuyiSDK 官方的
              人工测试，被标记为「RuyiSDK Certified」；其余取值（含字符串 `"false"`、`"true"`
              等非布尔值）均会被拒绝，而非当作已认证处理。相关认证的溯源信息（谁在何时、针对
              哪个版本进行了测试等）由软件源的 Git 历史记录承载，故此处不再重复记录。
...
* `binary` 仅在 `kind` 含有 `binary` 时有意义，表示适用于二进制包的额外信息。其类型为列表，每条记录：
    ...
    - `metadata` 是可选的子表，包含基于该宿主选择额外推导或关联的元数据。目前定义如下字段：
        - `install_size`：可选整数，表示该选择对应的解压后文件的近似大小（字节）。实际磁盘占用会因文件系统块大小、inode 开销等因素而不同。
* `blob` 仅在 `kind` 含有 `blob` 时有意义，表示适用于二进制数据包的额外信息。其中：
    ...
    - `metadata` 是可选的子表，含义与 `binary` 的 `metadata` 字段相同。
